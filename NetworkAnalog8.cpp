...
string code(int e,char* message_new,int n){
    //cout<<message_new<<endl;
    //char* message_ = (char*)message_new.data();
    int i = strlen(message_new);
    //cout<<message_new<<endl;
    int m=0;
    int message_code[100] = {};

    char message[50] = "qweasdzxc";
    //int i = 9;

    for(int q = 0;q<i;q++){
        message[q] = message_new[q];
    }  


    for(m=0;m < i;m++){
    message_code[m] = rsa(message[m]-'@',43,n);     
    }
    char mesage_final[50] = "";
    //cout<<"加密完成"<<endl;


    for(m=0;m < i;m++){
        mesage_final[m] = rsa(message_code[m],2227,n)+'@';
        //printf("%c",mesage_final[m]);
    }
    //cout<<"解密完成"<<endl;
    

    string mes_final;
    mes_final.append(mesage_final);
    return mes_final;
}

/*
string decode(int e,string message_new,int n){
    //cout<<message_new<<endl;
...
void send(Node* self,Node* temp1){
    ...
    message_target = code(43,message,71*73);
    ...
    temp1->data.append(self->name);
    temp1->data.append("发来消息:");
    temp1->data.append(message_target);
    //cout<<"解密后："<<code(temp1->secret_key,message_target,71*73)<<endl;
    //temp1->data.append(code(temp1->secret_key,message_target,71*73));
    return;
}
